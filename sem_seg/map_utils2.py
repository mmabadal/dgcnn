...
def get_info_map(info_map, info_world):
    ...
    if len(pipe_inst_map_list)>0:
        pred_pipe_map = np.vstack(pipe_inst_map_list)

    if len(pipe_inst_world_list)>0:
        pred_pipe_world = np.vstack(pipe_inst_world_list)

    if len(pipe_inst_map_list)>0 and len(pipe_inst_world_list)>0:

        pred_pipe_map = np.vstack((pred_pipe_map, pred_pipe_world))
    elif len(pipe_inst_world_list)>0:
        pred_pipe_map = pred_pipe_world
...
def clean_map(info_map, count_thr):
    ...
    for i in sorted(valve_del_list, reverse=True):  # delete valve
