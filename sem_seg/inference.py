...
"python inference_online_all.py --path_data a/b/c --path_out a/b/c --path_cls meta/class_or.txt --model_path RUNS/test_indoor --points_sub 128 --points_proj 512 --test_name test_name"
            ...
            if re.search("\.(txt)$", file[1]):          # if its a txt       
                ...
                if data_label_full.shape[0]> 2000: # 200000 check pointcloud points, a good PC has ~ 480k points  -> 200K for unfiltered PC, 2k for filtered PC
                    ...
                    if empty3==False:
                        ...
                        path_out3_array = os.path.join(path_out, os.path.basename(filepath)[:-4]+'_info_ref_array.npy')
