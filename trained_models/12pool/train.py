...
def train():
  with tf.Graph().as_default(), tf.device('/gpu:0'):
    ...
    for epoch in range(MAX_EPOCH):
      ...
      stop = early_stopping(loss_t_list, loss_val_list, 9999)
      if stop:
  ...
  save_path = saver.save(sess, os.path.join(LOG_DIR, "model_last.ckpt"))
  save_path = saver.save(best_sess, os.path.join(LOG_DIR, "model_best.ckpt"))
